...
const createWalletServiceHandlers = (
  ...
  if (forMali) {
    return {
      ...
      async createWallet(nameSpace: string, passPhrase?: string, seed?: ReadonlyArray<string>): Promise<void> {
        if (seed) {
    ...
    return {
      createWallet: (call: any, cb: (error: any, value: any) => void): void => {
        ...
        if (call.request.seed) {
          walletRepo.createFromSeed(nameSpace, passPhrase, call.request.seed)
            .then((isSuccess) => {
